...
## 7. Agent 化架构扩展方案（2026-03）

为支持“意图识别 + 工具调用 + 工作流 + 流式输出 + 人工审批 + 多轮联动”，后端从单体 controller 组织升级为模块化架构：

*   `rag`：仅负责 RAG 基础能力（向量检索、召回、重排、上下文返回）
*   `knowledge`：负责知识库、文档、Chunk 管理与权限过滤
*   `engine`：负责查询重写、意图识别、检索拼装、会话状态机、暂停恢复、工作流编排，并统一调度 `rag` 与 `skill`
*   `user`：负责用户登录、角色鉴权、JWT、RBAC
*   `admin`：负责管理员配置、用户创建、资源授权、审计查询
*   `skill`：负责工具注册、参数校验、调用执行与结果回传

### 7.1 目标能力
1. RAG 检索命中后，自动提取工具所需参数草稿。
2. 前端可人工审查并修改参数，再确认执行。
3. 执行后继续当前会话，不中断多轮上下文。
4. 超级管理员可审计所有用户查询和工具调用。

### 7.2 两阶段落地
*   **阶段 A（可运行）**：先打通 `engine + skill` 的审批流闭环（含 SSE 事件：`tool_draft`）。
*   **阶段 B（可治理）**：补齐 `user + admin` 的角色治理和审计统计，形成可管可控的生产架构。
